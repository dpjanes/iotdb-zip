...
const read = (_self, done) => {
    ...
    case "utf8": 
        ...
        break;
    default:
        assert.ok(false, `${method}: unknown self.document_encoding="${self.document_encoding}"`)
    }
...
/**
 */
const read_json_p = _path => _.promise.denodeify((_self, done) => {
    const self = _.d.clone.shallow(_self);
    const method = "lib.read.json.p";

    const json_path = _path || self.path;
    assert.ok(_.is.String(json_path), `${method}: path is required`)

    _.promise.make(self)
        .then(sd => _.d.add(sd, "path", json_path))
        .then(exports.read.json)
        .then(_.promise.done(done, self, "json,exists"))
        .catch(done)
})
